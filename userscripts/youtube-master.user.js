...
(function() {
    ...
    function deleteMembersOnlyVideos() {
        ...
        const elements = Array.from(document.querySelectorAll('p')).filter(
            el => el.textContent.trim() === "Members only"
        );
    ...
    function removeShortsRows() {
        const shortsRows = Array.from(document.querySelectorAll('ytd-rich-section-renderer, ytd-rich-shelf-renderer'))
            .filter(el => el.textContent && /Shorts/i.test(el.textContent));
    ...
    function ensureStyle() {
        ...
        style.appendChild(document.createTextNode(
            "ytd-video-renderer[data-excluded=\"true\"] { display: none !important; }\n            #yt-search-exclusion-tip { will-change: transform, opacity; }"
        ));
...
min-height: " + height + "px !important; max-height: none !important; height: " + height + "px !important }";
    ...
    async function applyMaxQualitySettings() {
        ...
        if (typeof GM != 'undefined' && GM.getValue && GM.setValue) {
            ...
            if (maxQualitySettings.overwriteStoredSettings || !settingsSaved) {
                ...
                await Promise.all(
                    Object.keys(maxQualitySettings).map(k => { let newval = GM.getValue(k); return newval.then(v => [k,v]); })
            ...
            debugLog(Object.entries(maxQualitySettings).map(([k,v]) => k + " | " + v).join(", "));
        }
    }
    ...
    // 4. Apply Max Quality settings and run initial page features
