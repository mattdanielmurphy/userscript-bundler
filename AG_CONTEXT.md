...
- YouTube Master Script features: Refresh on unavailable, toggle thumbnails, remove members-only (enhanced for sidebar), hide shorts, hide low view videos (<1k), get transcript button (ultra-fast extraction with smart waiting & innerText fallback), search exclusion, max quality, ignore number keys (seeking), and **URL parameter-driven Highlight Reel engine** (supports `?highlights=...`, `&reel=...`, timestamp intervals, labels, JSON/Base64, scrubber heatmaps, auto-skipping, and right-click to copy shareable highlight URLs).
...
- **Territorial & Land Acknowledgements Stripper**: Automatically detects and strips territorial and land acknowledgement banners, footers, sections, and callouts across all websites (`*://*/*`) using instant preemptive CSS injection (zero FOUC) and intelligent DOM/heading analysis with false-positive protection for news articles.
- **DOM Action Recorder & Automation Generator**: Cross-domain, multi-page DOM action recorder that survives page loads, full form submissions, and redirects across different origins via `GM_getValue`/`GM_setValue`. Captures debounced inputs, clicks, dropdowns, navigation, and keypresses with multi-strategy locators (`data-testid`, name, semantic id, label, cssPath, xpath), a floating draggable HUD (`Alt+Shift+R`), and 1-click export to runnable Playwright/Puppeteer automation scripts.
- **Amazon Brand Allowlist & Product Filter**: 4-tier allowlist brand verification engine and keyword filter across global Amazon domains (`.com`, `.ca`, `.co.uk`, etc.). Integrates 24h cached remote allowlist sync (Chris Mosley community list + seed list), structural on-page signals (Brand Store `/stores/` links, Sold/Ships by Amazon, Choice/Best Seller badges), linguistic heuristics (detects spam ALL-CAPS 5-9 letter strings, verifies Title Case vowel ratios and corporate suffixes), local user whitelist overrides with an in-page floating control panel, live stats, hard-hide/soft-dim toggles, 1-click whitelist buttons on search cards, a **1-click Show All Items toggle button** in the top search bar and zero-results banner, an **automated next-page pagination engine** with countdown notice, cancel button, and 10-hop safety guard when filtering yields 0 products on a page, **flexible hyphen/space/punctuation delimiter matching** (`pull-out` matches `Pull Out` / `pullout`), **comma-separated multi-term support in 'Require'** (`dual, step`), **sponsored banner (`.AdHolder`) filtering**, and **accurate keyword vs brand breakdown stats in the UI**.
