...
- **Amazon Brand Allowlist & Product Filter**: 4-tier allowlist brand verification engine and keyword filter across global Amazon domains (`.com`, `.ca`, `.co.uk`, etc.). Integrates 24h cached remote allowlist sync (Chris Mosley community list + seed list), structural on-page signals (Brand Store `/stores/` links, Sold/Ships by Amazon, Choice/Best Seller badges), linguistic heuristics (detects spam ALL-CAPS 5-9 letter strings, verifies Title Case vowel ratios and corporate suffixes), local user whitelist overrides with an in-page floating control panel, live stats, hard-hide/soft-dim toggles, 1-click whitelist buttons on search cards, a **1-click Show All Items toggle button** in the top search bar and zero-results banner, and an **automated next-page pagination engine** with countdown notice, cancel button, and 10-hop safety guard when filtering yields 0 products on a page.
