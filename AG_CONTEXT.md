...
- **ContentConnections Enhancements**: Hides whiteboard/menu, adds "Yes (Show Sol.)" and "No (Skip Sol.)" buttons, and automates next steps based on correctness choice. Includes a **Canvas Capture Automation** (triggered by `Opt+D` on PlayerView pages) to automatically seek and download slides as images.
