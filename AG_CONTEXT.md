...
- **ContentConnections Enhancements**: Hides whiteboard/menu, adds "Yes (Show Sol.)" and "No (Skip Sol.)" buttons, and automates next steps based on correctness choice. Includes a **Canvas Capture Automation** (triggered by `Opt+D` on PlayerView pages) to automatically seek and download slides as images, and a **Proper Dark Mode** (canvas inversion, iframe background overrides, general CSS theme, and image inversion for consistency). Includes enhanced practice UI buttons and layout fixes for media controls. **Fixed lesson indexing to ignore non-lesson items like quizzes/tests for accurate 'L#' labeling.**
