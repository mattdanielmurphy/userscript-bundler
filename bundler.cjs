...
function generatedBanner(groupEntry) {
	const sourceDir = "~/projects/userscript-bundler/userscripts/"
	const sourceFiles = (groupEntry.files || []).map(f => `     ${sourceDir}${f}`).join("\n")
	return [
		"/* ═══════════════════════════════════════════════════════════════════════════",
    ...
    console.error("❌ [Bundler] Critical Error:", e);
		...
		if (groupedEntries.length > 0) {
			...
			for (const groupEntry of groupedEntries) {
				...
				standaloneCode += generatedBanner(groupEntry) + compatCode + concatenatedBody
				...
				// Ensure the generated file is writable before overwriting (it is
				// left read-only after each build to discourage manual edits).
				try { fs.chmodSync(outputPath, 0o644) } catch (e) {}
				fs.writeFileSync(outputPath, standaloneCode, "utf8")
				// Enforce read-only so the generated file cannot be edited in place.
				fs.chmodSync(outputPath, 0o444)
				const fileStats = fs.statSync(outputPath)
				console.log(`  ✅ Compiled: ${outputFilename} (${(fileStats.size / 1024).toFixed(2)} KB, read-only)`)
			}
		}
